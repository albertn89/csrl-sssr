...
class ObservationEncoder(nn.Module):
    def __init__(self, num_inputs, dim, channel=32):
        ...
        for i in range(dim):
            for j in range(dim):
                self.position_encoder[:, i, j] = torch.tensor(
                    [
                        (i - int(dim / 2)) / int(dim / 2),
                        (j - int(dim / 2)) / int(dim / 2),
                    ]
                ).to(device)
...
class QNetwork(nn.Module):
    def __init__(self, num_inputs, action_res, max_act_res, hidden_dim):
        ...
        self.interpolate = nn.Upsample(
            scale_factor=self.interpolate_rate, mode="bicubic", align_corners=True
        )
    ...
    def forward(self, state, action):
        action = self.interpolate(
            action.reshape(-1, 2, self.action_res, self.action_res)
        )
        if self.merge:
...
class GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, action_res, max_action_res, residual=False, bias=0):
        ...
        if residual:
            ...
            self.upsample = nn.Upsample(
                scale_factor=64 / action_res, mode="bicubic", align_corners=True
            )
            self.residual = True
        else:
            self.residual = False
    ...
    def forward(self, state, coarse_action=None):
        ...
        std = torch.clamp(
            self.soft_plus(SOFT_PLUS_K * self.std(x) + SOFT_PLUS_B), MIN_STD, MAX_STD
        ).reshape(x.shape[0], -1)
        if self.residual:
